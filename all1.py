...
def temps():
    ...
    for z in range(0,len(T4)):
        if T4[z]>=-200 and T4[z]<=300:
            ...
            temp.append(T4[z])
        z=z+1
    dict1 = {"name": "temp_PT100", "value": temp}
    return dict1
...
def take_hum():
    ...
    #d = {'date': time.strftime('%m/%d/%y'),
     #    'time' : time.strftime('%H:%M'),
      #   'tem':float("{:.2f}".format(temperature)),
       #  'hum' : float("{:.2f}".format(humidity))}
    dict1 = {"name": "HUM", "value":float("{:.2f}".format(humidity))}
    return dict1

def take_temp():
        humidity, temperature =  Adafruit_DHT.read_retry(DHT_SENSOR, DHT_PIN)
        dict1 = {"name": "Temp", "value": float("{:.2f}".format(temperature))}
        return dict1
def millis():
